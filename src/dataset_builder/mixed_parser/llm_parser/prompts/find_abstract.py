"""Prompts to find and extract the abstract of a paper, the {input} variable is expected in the QUERY prompt"""

from pydantic import BaseModel, Field

class OutputSchema(BaseModel):
    is_abstract: bool = Field(examples=[True, False])
    abstract: str = Field(examples=["El presente trabajo de diploma documenta el desarrollo de una solución al problema de ..."])

# Constants to store the names of the json fields in the output, to avoid repetition
IS_ABSTRACT_FIELD = "is_abstract"
...
This is an early page of a thesis (within the first 10 pages). Your task is to determine whether the page contains the abstract of the thesis, and extract it if and only if it does. The abstract is a summary that typically includes the research problem, methodology, and key findings.
...
- "{IS_ABSTRACT_FIELD}": true or false, whether or not the page contains an abstract
- "{CONTENT_FIELD}":
        - "" if {IS_ABSTRACT_FIELD} is false
        - if {IS_ABSTRACT_FIELD} is true, then the text of the abstract without modifications
...
{{
    ...
    "{CONTENT_FIELD}": "El presente trabajo de diploma, propone dar continuidad  al desarrollo del visor
cartográfico para la web \\"OpenLatinoViewer\\". Surge de la necesidad de extender este visor,
para incorporar nuevas herramientas que permitan mayor interacción con el sistema. Por
ejemplo, se propone la implementación de herramientas que permitan realizar consultas
espaciales y avanzadas y que además permita la visualización y procesamiento de los
...
def FIND_ABSTRACT_CONTEXT() -> str: 
    """Returns a context that is expected to be prepende to the query"""
    return CONTEXT_PROMPT

def FIND_ABSTRACT_QUERY(input: str) -> str: 
    """Returns the query part of the prompt, expected to be appended to the context"""
    return f"""
### Real Data ###
##### Input #####
{input}
